...
# GLOBAL tracker: atoms that already got the +1
updated_atoms_once = set()
for item_op in orbital_participation:
    for key_op, value_op in item_op.items():

        # Track elements already updated for this key_op (e.g. H-H)

        for item_vop in value_op:
            for key_vop, value_vop in item_vop.items():
                element = key_vop
                orbitals_to_update = value_vop

                # Skip if this atom already got +1
                if element in updated_atoms_once:
                    continue

                if element in areecm_digital_dictionary:
                    total_electrons, orbitals_dict = areecm_digital_dictionary[element]

                    if element not in new_areecm_dict:
                        new_areecm_dict[element] = [total_electrons, {orb: vals.copy() for orb, vals in orbitals_dict.items()}]

                    for orbital in orbitals_to_update:
                        if orbital in new_areecm_dict[element][1]:
                            old_pmax = new_areecm_dict[element][1][orbital][1]
                            multiplied_value = total_electrons * old_pmax
                            added_value = multiplied_value + 1
                            new_pmax = added_value / total_electrons
                            new_areecm_dict[element][1][orbital][1] = round(new_pmax, 3)
                    updated_atoms_once.add(element)
...
print(new_areecm_dict, 'new_areecm_dict')


# Visualizer Code:
if visualizer_mode_confirmation == 'YES' or visualizer_mode_confirmation == 'Y':
    bond_length_dictionary = {}
    for item in bond_length_list:
        for bond, pm in item.items():
            bond_length_dictionary[bond] = pm
    ...
    for item_orl in overlap_radius_list:
        for key_orl, R in item_orl.items():
            ...
            radius1 = electron_cloud_radii[ele_1_orl]
            radius2 = electron_cloud_radii[ele_2_orl]

            bond_length_pm = bond_length_dictionary[key_orl]
            bond_length_A = bond_length_pm / 100.0
            ...
            fig, ax = plt.subplots()
            figures.append(fig)  # keep reference
            ax.set_aspect('equal')

            ax.add_artist(plt.Circle((x1, y), radius1, color='blue', alpha=0.3))
            ax.add_artist(plt.Circle((x2, y), radius2, color='red', alpha=0.3))

            ax.plot(x1, y, 'bo', markersize=6)
            ax.plot(x2, y, 'ro', markersize=6)


            def draw_orbitals(ax, x_center, element, color):
                orbitals = areecm_digital_dictionary[element][1]
                Rcloud = electron_cloud_radii[element]
                for orbital in orbitals:
                    if orbital in orbital_distances:
                        _, outer = orbital_distances[orbital]
                        if outer <= Rcloud:
                            ax.add_artist(plt.Circle((x_center, 0), outer, fill=False, color=color, alpha=0.5))

            draw_orbitals(ax, x1, ele_1_orl, 'blue')
            draw_orbitals(ax, x2, ele_2_orl, 'red')

            ax.set_xlim(-radius1 * 2, x2 + radius2 * 2)
            ax.set_ylim(-max(radius1, radius2) * 2, max(radius1, radius2) * 2)
            ax.set_title(f"{key_orl} bond (overlap radius = {R:.2f} Å)")
            ax.set_xlabel("X (Å)")
            ax.set_ylabel("Y (Å)")
            ax.grid(False)
    plt.show()
